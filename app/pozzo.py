# py -m PyInstaller --onefile --noconsole --add-data "index.html;." pozzo.py
# pyinstaller --onefile --noconsole --add-data "index.html:." pozzo.py
...
CARD_NAMES = {
    ...
    6: "VI (Standard 6)", 7: "VII (Standard 7)", 8: "VIII (Standard 8)", 9: "IX (Standard 9)", 10: "X (Standard 10)",
    11: "XI (Standard 11)", 12: "XII (Standard 12)", 13: "XIII (Standard 13)", 14: "XIV (Standard 14)", 15: "XV (Standard 15)",
...
CARD_DESCRIPTIONS = {
    1: "Lower Tier. Twin Immunity: If two are revealed, shuts down round execution entirely and restores life from the Well.",
    2: "Lower Tier. Twin Immunity: If matched with another Tier II, grants absolute round immunity to both holders.",
    3: "Lower Tier. Twin Immunity: If matched with another Tier III, grants absolute round immunity to both holders.",
    4: "Lower Tier. Twin Immunity: If matched with another Tier IV, grants absolute round immunity to both holders.",
    5: "Lower Tier. Twin Immunity: If matched with another Tier V, grants absolute round immunity to both holders.",
    6: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    7: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    8: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    9: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    10: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    11: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    12: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    13: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    14: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    15: "Standard Numeric Rank. Safe from specialized defensive reactions but vulnerable to falling lowest.",
    16: "Major Arcana. Defensive: Completely blocks incoming trade requests from your right-hand neighbor.",
    17: "Major Arcana. Defensive: Blocks trades. Forces the initiator to swap with a random third party instead.",
    18: "Major Arcana. Defensive: Blocks trades. Forces the initiator to trade with the player directly after you.",
    19: "Major Arcana. Defensive: Blocks trades. The initiator is mauled, losing 1 life directly into the Well pool.",
    20: "Major Arcana. Ultimate: Revealed before trading begins. Halts all trades. Declare 'Well!' to steal life."
}
...
def generate_procedural_art(card_id):
    """Generates standard card boundaries dynamically for cards 6-15."""
    rank_center = f"{card_id}".center(3)
    return f"""      .----------------------.
      | {card_id:<2}                   |
      |                      |
      |                      |
...
class PozzoGame:
    ...
    def write_card_art(self, card_id):
        if card_id in CARD_ART:
            self.bridge.write(CARD_ART[card_id] + "\n")
        else:
            self.bridge.write(generate_procedural_art(card_id) + "\n")
    ...
    def play_round(self):
                    ...
                    if target_card == 16:
                        ...
                        if other_players:
                            ...
                            if p == "You":
                                ...
                                self.bridge.write(f"Your card was swept up in the redirection! Your new card is: {card_str(hands['You'])}\n")
                                self.write_card_art(hands['You'])
                        else:
                        ...
                        if next_after_target != p:
                            ...
                            if p == "You":
                                ...
                                self.bridge.write(f"Your card caught fire and swapped! Your new card is: {card_str(hands['You'])}\n")
                                self.write_card_art(hands['You'])
                        else:
                        ...
                        if p == "You":
                            self.bridge.write(f"Trade complete! Your new card is: {card_str(hands['You'])}\n")
                            self.write_card_art(hands['You'])
                        elif target == "You":
                            self.bridge.write(f"Your card was forcefully taken! Your new card is: {card_str(hands['You'])}\n")
                            self.write_card_art(hands['You'])
                        else:
                            self.bridge.write(f"Cards were swapped between {p} and {target}.\n")
                    time.sleep(1.5)
        ...
        if deck:
            omen_card = deck.pop(0)
            self.bridge.write(f"\n[OMEN] The Executioner turns over the top card of the deck... It's the *{card_str(omen_card)}*!\n")
            self.write_card_art(omen_card)
            time.sleep(1.5)

            omen_matches = []
            for p in active:
                if hands[p] == omen_card:
                    immune_players.add(p)
                    omen_matches.append(p)
        ...
        for val, players_with_card in card_counts.items():
            if len(players_with_card) >= 2:
                if val == 1:
                    self.bridge.write(f"\n[TWIN FOOLS] Two players uncover I (The Fool)!\n")
                    ...
                    self.bridge.write(f"\n[TWIN TRIGGER] Two players hold {card_str(val)} and are immune to execution!\n")
    ...
    while True:
        ...
        if choice == "1":
            ...
            while True:
                bridge.clear()
                bridge.write("====================================\n")
                bridge.write("          DECK COMPENDIUM           \n")
                bridge.write("====================================\n")
                for i in range(1, 21):
                    bridge.write(f" {i:<2}] {CARD_NAMES[i]}\n")
                bridge.write(" 0 ] Return to Main Menu\n")
                bridge.write("====================================\n")
                v_choice = bridge.read("Select a card ID to view properties: ").strip()
                if v_choice == "0":
                    break
                if v_choice.isdigit() and 1 <= int(v_choice) <= 20:
                    c_id = int(v_choice)
                    bridge.clear()
                    bridge.write(f"=== {CARD_NAMES[c_id].upper()} ===\n")
                    bridge.write(f"Rank Value: {c_id}\n")
                    bridge.write(f"Description: {CARD_DESCRIPTIONS[c_id]}\n\n")
                    if c_id in CARD_ART:
                        bridge.write(CARD_ART[c_id] + "\n\n")
                    else:
                        bridge.write(generate_procedural_art(c_id) + "\n\n")
                    bridge.read("Press Enter to return to index...")
                else:
                    bridge.write("Invalid option. Choose a rank from 1 to 20.\n")
                    time.sleep(1)
        elif choice == "5":
            bridge.clear()
